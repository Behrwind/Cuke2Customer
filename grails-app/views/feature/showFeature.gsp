  ...
  <body>
    ...
    <g:each in="${feature.scenarios}" var="scenario" status="scenarioIndex">
      <div class="scenario indented ${scenarioIndex % 2 == 1 ? 'odd' : ''}">
        <p class="title">
          <a name="scenario-${scenarioIndex}"></a>${scenarioIndex + 1}. ${scenario.title} <c2c:jiraUrlFromTags
            tags="${scenario.tags}"/>
      ...
      <div id="commentsExplanation" class="jira indented">
        <p>
          <g:if test="${!comments.empty}">
            ...
            <a href="${c2c.jiraUrl(feature: feature)}" target="_blank"><g:message code="join.jira.discussion"/></a>
          </g:if>
          <g:else>
            <g:message code="no.comments.for"/> ${feature.jiraId}.<br/>
            <a href="${c2c.jiraUrl(feature: feature)}" target="_blank"><g:message code="create.comment.in.jira"/></a>
          </g:else>
        </p>
      </div>
